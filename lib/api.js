...
export const logout = async () => {
  try {
    ...
    console.warn('Erreur lors de la déconnexion serveur, nettoyage côté client effectué:', error);
    
    // Nettoyage côté client même en cas d'erreur serveur
    cleanupClientData();
    
    // Retourner une réponse de succès pour permettre la déconnexion côté client
    return {
      success: true,
      message: 'Déconnexion effectuée (nettoyage côté client)',
      fallback: true
    };
  }
};
...
/**
 * Nettoyage complet des données côté client
 */
const cleanupClientData = () => {
  try {
    ...
    if (typeof window !== 'undefined') {
      ...
      // Nettoyage des cookies côté client (si possible)
