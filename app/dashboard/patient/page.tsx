...
export default function PatientDashboardPage() {
  ...
  useEffect(() => {
    ...
    if (parsedUser.role === "patient") {
      setPatientCode("123456") // Code par défaut, sera mis à jour par l'API
    }
  }, [router])
  ...
  const handleResetCode = async () => {
    ...
    setIsLoading(true)
    try {
      const response = await resetCodePatient({}) // Call API without new code parameter
      setSuccess((response as any)?.message || "Code secret réinitialisé avec succès")
      if ((response as any)?.nouveauCode) { // Update displayed code if API returns it
        ...
        <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-gray-600">Bienvenue, {user.nom} {user.prenom}</p>
            </div>
